...
export async function POST({ request }: { request: Request }) {
  return gestisciPratica(request, PORTALI_PARTNER['extreme-computer'], {
    ...
    notaLavorazione:
      "Partner: EXTREME COMPUTER di Cinnella Luca (Potenza, P.IVA 01327710768, direzione@extremecomputer.it). Impresa individuale attiva dal 1998, un addetto, ATECO 26.20.00. Canale PagaRent, unico esposto nel portale: se la pratica va ruotata altrove (ReteRent/Grenke o BCC) la decisione resta a MCF. Beni: PC e workstation, server e NAS, gruppi di continuita', networking, videosorveglianza, software. Verificare lo stato del censimento fornitore prima di promettere il pagamento diretto: fino al perfezionamento le pratiche le carica MCF.",
  });
}
