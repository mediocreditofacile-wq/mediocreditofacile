...
import './simulatore-expo-energia.css';

// Simulatore in stile ReteRent per l'area partner Expo Energia:
// due tabelle coefficienti (ESG fotovoltaico / Pioneer hardware) e doppio calcolo,
// dal prezzo di vendita ai canoni su tutte le durate e dal canone al prezzo.

type Tabella = 'esg' | 'pioneer';
...
const TABELLE: Record<Tabella, { label: string; hint: string; min: number; max: number; durate: number[] }> = {
  ...
  pioneer: {
    label: 'Pioneer — Hardware e casse',
    hint: 'Casse automatiche, POS, sistemi di pagamento, software. Canone mensile con fatturazione trimestrale anticipata.',
    min: 500,
    max: 100000,
...
export default function SimulatoreExpoEnergia() {
  const [tabella, setTabella] = useState<Tabella>('esg');
  ...
  const [durataCanone, setDurataCanone] = useState(60);
  ...
  const handleCalcolaCanoni = () => {
    ...
    if (typeof window !== 'undefined' && (window as any).dataLayer) {
      (window as any).dataLayer.push({ event: 'calcolo_eseguito', tool: 'portale_expo_energia', tabella, valore: num });
    }
  };
  ...
  return (
    <div class="see">
      ...
      <div class="see__panel">
        <div class="see__panel-head">Parametri dell'offerta</div>
        <div class="see__panel-body">
          <div class="see__tabelle">
            {(Object.keys(TABELLE) as Tabella[]).map((t) => (
              <button
                key={t}
                type="button"
                class={`see__tabella-btn ${tabella === t ? 'see__tabella-btn--active' : ''}`}
          ...
          <p class="see__hint">{cfg.hint} Importi da {eur(cfg.min)} a {eur(cfg.max)}, imponibili IVA.</p>
        </div>
      </div>
      ...
      <div class="see__cols">
        ...
        <div class="see__panel">
          <div class="see__panel-head">Simulazione a partire dal prezzo di vendita</div>
          <div class="see__panel-body">
            <label class="see__label" for="see-prezzo">Prezzo di vendita (€)</label>
            <div class="see__input-row">
              <input
                id="see-prezzo"
                type="text"
                inputMode="decimal"
              ...
              />
              <button type="button" class="see__btn" onClick={handleCalcolaCanoni}>Calcola canone</button>
            </div>

            {fuoriFascia && (
              <p class="see__warn">Importo fuori tabella: per questa tipologia il prezzo deve stare tra {eur(cfg.min)} e {eur(cfg.max)}. Per importi superiori contatta Mediocredito Facile.</p>
            )}

            {canoni && (
              <>
                <p class="see__grid-title">Scegli il numero di canoni</p>
                <div class="see__grid">
                  {canoni.map(({ durata, canone }) => (
                    <button
                      key={durata}
                      type="button"
                      class={`see__cell ${durataScelta === durata ? 'see__cell--active' : ''} ${canone === null ? 'see__cell--na' : ''}`}
                      onClick={() => canone !== null && setDurataScelta(durata)}
                      disabled={canone === null}
                    >
                      <span class="see__cell-durata">{durata} mesi</span>
                      <span class="see__cell-canone">{canone !== null ? eur(canone) : 'n.d.'}</span>
                    </button>
                  ))}
                </div>
                <p class="see__nota">Canone mensile indicativo, imponibile IVA, salvo delibera della società di noleggio.</p>
              </>
            )}
          </div>
        </div>
        ...
        <div class="see__panel">
          <div class="see__panel-head">Simulazione a partire dal canone</div>
          <div class="see__panel-body">
            <div class="see__canone-row">
              <div class="see__canone-field">
                <label class="see__label" for="see-canone">Canone mensile (€)</label>
                <input
                  id="see-canone"
                  type="text"
                  inputMode="decimal"
              ...
              <div class="see__canone-field">
                <label class="see__label" for="see-durata">Numero di canoni</label>
                <select
                  id="see-durata"
                  class="see__select"
                  value={durataCanone}
            ...
            <button type="button" class="see__btn see__btn--full" onClick={handleCalcolaPrezzo}>Calcola prezzo di vendita</button>

            {canoneCalcolato > 0 && (
              prezzoRicavato !== null ? (
                <div class="see__prezzo-card">
                  <span class="see__cell-durata">Prezzo di vendita</span>
                  <span class="see__prezzo-value">{eur(prezzoRicavato)}</span>
                </div>
              ) : (
                <p class="see__warn">Nessun prezzo in tabella corrisponde a questo canone su {durataCanone} mesi. Prova a cambiare durata o importo.</p>
              )
            )}
          </div>
        </div>
      ...
      {/* Stampa simulazione */}
      {canoni && (
        <div class="see__actions">
          <button type="button" class="see__btn see__btn--ghost" onClick={handleStampa}>Stampa simulazione</button>
          <span class="see__actions-hint">La stampa è pulita e senza riferimenti interni: puoi consegnarla al cliente.</span>
        </div>
      )}

      {/* Area stampa (visibile solo in stampa) */}
      {canoni && (
        <div class="see__print">
          <div class="see__print-head">
            <span class="see__print-logo"><b>Medio</b>credito <em>Facile</em></span>
            <span class="see__print-claim">L'OFFICINA DEL CREDITO</span>
          </div>
          <h2>Simulazione noleggio operativo</h2>
          <p>Bene: {tabella === 'esg' ? 'Impianto fotovoltaico' : 'Hardware / sistemi di cassa'} — Prezzo di vendita {eur(prezzoCalcolato)} (imponibile IVA)</p>
          <table>
            <thead><tr><th>Durata</th><th>Canone mensile</th></tr></thead>
            <tbody>
              {canoni.filter((c) => c.canone !== null).map(({ durata, canone }) => (
                <tr key={durata} class={durataScelta === durata ? 'see__print-sel' : ''}>
                  <td>{durata} mesi</td>
                  <td>{eur(canone!)}</td>
                </tr>
              ))}
          ...
          <p class="see__print-footer">
            Mediocredito Facile — mediocreditofacile.it — +39 393 995 7840 — in collaborazione con Expo Energia Srl
          </p>
        </div>
      )}
    </div>
