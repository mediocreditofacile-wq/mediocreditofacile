...
- src/pages/tools/portali-clienti.astro → `/tools/portali-clienti`, indice dei portali pratiche partner. Aggiunto il 28/07/2026. Pagina PUBBLICA (nessun gate) ma noindex: si manda via WhatsApp o email al partner perche' raggiunga il proprio portale senza ricordare lo slug. Elenca SOLO i portali costruiti sul motore condiviso (oggi Expo Energia, STILO, Full Service) con card, settore e descrizione; NON espone credenziali. Le aree partner col solo simulatore (Arca Energia, AGE, EnergyTeam, GIERRE, Econocom PA, Gruppo Barone, Datron, Duplex, Lo Martire) restano fuori. L'elenco e' l'array `PORTALI` in testa al file: aggiungere una voce li' quando nasce un nuovo portale. Nell'hub `/tools/` le tre card dei singoli portali sono state sostituite da un'unica card "Portali Clienti" che punta qui.
