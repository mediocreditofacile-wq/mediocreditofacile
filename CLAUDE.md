...
- src/pages/tools/full-service.astro → portale partner FULL SERVICE S.R.L. (Maria Orsola Corcione, referente commerciale; legale rappresentante Michele Marinelli; P.IVA 02788160360; sede Fiorano Modenese, unità locale Cadelbosco di Sopra). Login con EMAIL + password (user: `commerciale@fullserviceagency.biz`, password: `fullservice`, localStorage key `mcf_fullservice_auth`, noindex). Aggiunto il 24/07/2026 clonando il portale STILO ma con le **sole tabelle BCC Rent&Lease, niente Grenke**: `SimulatorePortale` con `tabelle={['bcc-lo','bcc-lf','bcc-ff','bcc-zero']}` (locazione operativa, leasing, finanziamento finalizzato, campagna tasso zero 10 mesi), `classeBcc={2}` (Information technology: PC, server, monitor), etichette bene "Bene strumentale IT / attrezzatura". Menu prodotto visibile (più tabelle), documento extra in checklist "Offerta / preventivo al cliente finale", submit → `/api/submit-full-service`. Ratio interna (NON esposta in pagina): unico canale BCC Rent&Lease su beni IT e attrezzature. **Argomento di vendita in pagina**: hanno scartato Grenke perché addebita l'assicurazione in un'unica soluzione, mentre con BCC l'assicurazione all risk è già compresa nella rata mensile.
- src/pages/tools/unidima.astro → portale partner UNIDIMA SRL (Fabio Maresca, referente commerciale; amministratrice unica Florinda Carfora; P.IVA 04250321207; sede Bologna, deposito a Baronissi). Login con EMAIL + password (user: `info@unidima.it`, password: `unidima`, localStorage key `mcf_unidima_auth`, noindex). Aggiunto il 03/08/2026 clonando il portale FULL SERVICE. Simulatore su **due tabelle**: `tabelle={['pagarent','pioneer']}`, PagaRent aperta di default (durate 24-60 su entrambe). PagaRent e' il canale primario perche' il convenzionamento Grenke probabilmente non si fara'; etichetta bene "Ledwall / schermo LED", tipologie del form dedicate (ledwall indoor/outdoor, monitor, totem, struttura e installazione, player e software). Documento extra in checklist "Offerta / preventivo al cliente finale", submit → `/api/submit-unidima`, prefisso pratica `UD-`. Ratio interna (NON esposta in pagina): Unidima importa ledwall dalla Cina e rivende in Italia; censimento fornitore su ReteRent/Grenke (portale Contacto) in corso, quindi fino al perfezionamento le pratiche le carica MCF. In istruttoria pesano l'eta' della societa' (costituita a fine 2024, un solo esercizio operativo) e il fatto che Grenke paghi il fornitore a collaudo avvenuto. Scheda fornitore completa in ~/Desktop/PROGETTI/Clienti/Unidima Srl/.
- src/pages/tools/innovalux.astro → portale fornitore InnovaLux S.r.l. (Milano, P.IVA 13991550966, referente Antonia, contratti@innovalux.net). Login con EMAIL + password (user: `contratti@innovalux.net`, password: `innovalux`, localStorage key `mcf_innovalux_auth`, noindex). Aggiunto il 07/08/2026. **Non e' un portale pratiche: e' un portale PREVENTIVI.** Il fornitore inserisce i dati dell'impianto fotovoltaico e ottiene i due prospetti PDF (analitico a 9 sezioni + infografico a 3 pagine) che prima produceva Alberto a mano su richiesta via mail. Form: cliente (ragione sociale, comune e provincia, forma giuridica, riferimento preventivo), impianto (kWp, kWh accumulo, importo chiavi in mano, tipo installazione), consumi FACOLTATIVI (kWh/anno e euro/kWh da bolletta, profilo diurno o h24), durata (36/48/60/72 o "consigliata"). In uscita: riepilogo numeri a schermo, i due PDF scaricabili, mail ad Alberto con i PDF in allegato, record nello storico. Submit → `/api/preventivo-innovalux`, storico → `/api/preventivi-pv`, prefisso `IL-`, blob sotto `preventivi/innovalux/` (non `pratiche/`). **Avvisi di sola interfaccia** (mai nei PDF): forma giuridica "privato senza partita IVA" → il noleggio non e' percorribile, si va di detrazione 50%; "ditta individuale / persona fisica" → verificare partita IVA attiva e destinazione strumentale, occhio all'utenza domestica su immobile abitativo (errore gia' costato tempo su una pratica reale). Ratio interna (NON esposta): InnovaLux fornisce e installa impianti chiavi in mano; il portale ribalta sul fornitore il lavoro di preventivazione lasciando ad Alberto notifica e storico. Prossimo passo previsto ma NON fatto: upload dei documenti del cliente finale con checklist di completezza.
...
- public/tools/marotta/ → `/tools/marotta/`, tool finanziario della rete commerciale Marotta Evolution. **Non e' una pagina Astro**: e' una copia statica del repo `~/dev/marotta-tool` (index.html unico, PWA con sw.js e manifest), servita da `public/`. La fonte resta quel repo: si modifica li' e si allinea con `~/dev/marotta-tool/scripts/sync-mcf.sh`, poi commit e push qui. Spostato in casa il 04/08/2026 da Netlify, che ora fa 301 su questo URL; contestualmente rimosso da `vercel.json` il vecchio redirect `/tools/marotta` -> Netlify, che altrimenti creava un loop. Gate PIN `9999` lato client (stesso del tool Joker Store) + meta noindex; `/tools/marotta` e' gia' nella lista noindex della sitemap in `astro.config.mjs`. Usa le API `cerved.ts` e `credit-ai.ts` di questo stesso dominio.
- public/sorteggio-fantamicucci/ → `/sorteggio-fantamicucci/`, generatore del calendario asimmetrico della lega di fantacalcio privata di Alberto (Fantamicucci, dieci squadre). Aggiunto il 26/08/2026. **Non e' una pagina Astro**: e' un file HTML unico e autosufficiente servito da `public/`, con il suo CSS e il suo JS in pagina, cosi' non eredita ne' disturba il design system del sito. **Non e' un contenuto MCF**: e' un uso personale del dominio, quindi niente header, footer, navigazione, form o tracciamento, e nessun link che ci punti da nessuna pagina del sito — l'URL si conosce o non si trova. Nascosta con `<meta name="robots" content="noindex, nofollow, noarchive, nosnippet, noimageindex">`; **non va messa in robots.txt**, un Disallow impedirebbe a Google di leggere proprio il noindex, e la sitemap non la vede perche' @astrojs/sitemap elenca solo le pagine Astro. Sorgente e copia di lavoro in `~/Desktop/AREE/Fantacalcio/sorteggio_fantamicucci.html`: si modifica li' e si riporta qui riavvolgendo il file dentro l'involucro `<!DOCTYPE html>`/`<head>`/`<body>` (il sorgente parte dal `<title>` perche' nasce come artifact).
- src/pages/tools/portali-clienti.astro → `/tools/portali-clienti`, indice dei portali pratiche partner. Aggiunto il 28/07/2026. Pagina PUBBLICA (nessun gate) ma noindex: si manda via WhatsApp o email al partner perche' raggiunga il proprio portale senza ricordare lo slug. Due blocchi, entrambi array in testa al file (aggiungere li' una voce quando nasce un portale): `PORTALI_PRATICHE` = i portali sul motore condiviso (Expo Energia, STILO, Full Service, UNIDIMA); `AREE_PARTNER` = Marotta Evolution (`/tools/marotta/`, interno dal 04/08/2026) e le aree fotovoltaico (Arca Energia, AGE SRL, EnergyTeam, Econocom PA). NON espone credenziali. Restano fuori le aree non fotovoltaico: Edilizia GIERRE, Gruppo Barone, Datron, Datron ecommerce, Duplex, Lo Martire, simulazione-leasing. Nell'hub `/tools/` le card dei singoli partner (i tre portali + EnergyTeam, Econocom PA, Marotta) sono state sostituite da un'unica card "Portali Clienti" che punta qui; resta la card autonoma di Gruppo Barone.
- **Icone dentro le pagine .astro**: lo stile scoped di una pagina NON raggiunge lo span interno del componente `Icon` (ha il suo `data-astro-cid`), quindi `<Icon class="qualcosa" />` con `.qualcosa { font-size: 2.5rem }` non ha effetto e l'icona resta a 16px. Pattern corretto (quello di `ToolCard.astro`): wrapper `<span class="...">` per colore e margini, dimensione via prop `size={40}`. Corretto il 28/07/2026 sulle card riservate di `/tools/` (erano a 16px).
- **Motore condiviso dei portali partner** (Expo Energia, Stilo, Full Service, Unidima, prossimi): `src/data/portali-partner.ts` e' il registro server-side (slug, ragione sociale, password partner, prefisso id pratica) usato dalle API; `src/lib/pratiche-partner.ts` contiene tutta la logica di submit (id pratica, record JSON su Blob, mail Resend con link ai documenti, inoltro Zapier), cosi' gli endpoint `/api/submit-<slug>.ts` sono involucri di 10 righe che passano partner, documenti extra della checklist e nota di lavorazione interna. Il simulatore e' `src/components/tools/SimulatorePortale.tsx` (ex SimulatoreExpoEnergia) con prop `tabelle` (`'esg'`, `'pioneer'`, `'pagarent'`, `'bcc-lo'`; con una sola tabella lo switch si nasconde), `partner`, `etichetteBene` e `classeBcc`. **Per aggiungere un dealer**: voce nel registro + pagina `/tools/<slug>.astro` (clone di stilo.astro) + `/api/submit-<slug>.ts` + slug nella lista noindex della sitemap + voce nell'array `PORTALI` di `/tools/portali-clienti`. Le API `blob-upload`, `pratiche-expo` e `pratica-doc` sono gia' multi-partner: ogni partner vede e scarica solo la propria cartella `pratiche/<slug>/`, l'admin vede tutto. NOTA: la password del gate vive nello script della singola pagina (non nel registro) per non esporre nel bundle di un partner le credenziali degli altri; il registro resta server-side. **Distinta beni (multi-bene)**: una pratica e' un contratto con una o piu' righe bene. Il form di ogni portale usa il blocco a righe pilotato dall'helper client condiviso `src/scripts/beni-rows.ts` (`initBeniRows()`, prefisso-agnostico, lavora su ID stabili `#beni-rows`/`#beni-add`/`#beni-total`/`#beni-json`/`#importo` e data-attribute `[data-bene]`): aggiunge/rimuove righe, calcola il totale, sincronizza il campo nascosto `importo` col totale (cosi' la checklist dinamica continua a scattare) e serializza le righe nel campo nascosto `beni`. Lato server `pratiche-partner.ts` legge `beni` (fallback a riga singola dai campi flat per i bundle vecchi in cache), calcola l'importo TOTALE e lo usa per checklist, titolo Pipedrive e riepilogo; il record su Blob salva sia la distinta `beni[]` sia il riepilogo `bene{}` (importo = totale, descrizione unita, tipologia "Piu' beni (N)"), quindi la lista richieste dei portali resta compatibile senza modifiche. Durata e canone restano a livello contratto (un solo canone sul totale, il simulatore lavora sul totale). La mail elenca la distinta con il totale.
- **Coefficienti BCC Rent&Lease** (`src/data/bcc.ts`, 23/07/2026): BCC quota diversamente da Grenke, per **classe di rischio del bene** oltre che per durata e fascia di importo, e offre **tre prodotti**: locazione operativa (`lo`), locazione finanziaria/leasing (`lf`) e finanziamento finalizzato (`ff`). I coefficienti NON esistono come tabella nel planner `#_BCCRLplanner_LO+LF+FF_260226.xlsx`: escono dal modello (provvista → TIR → cash flow). Sono stati estratti ricalcolando l'intero planner in Python con la libreria `formulas` (le poche formule di sola visualizzazione vengono congelate al loro valore perche' il parser non le digerisce). **Per rigenerarli quando cambia il planner**: `python3 scripts/genera-coefficienti-bcc.py [percorso-planner]`, che rifa' tutte le griglie e si ferma da solo se il caso noto di verifica non torna. Config: canone/rata fissa mensile, anticipo primo canone, provvigione 3%, pagamento fornitore 2 giorni, ristorno spese SI, riscatto 1% su LO/LF e nessuno su FF. **Validazione**: 75.000 € a 60 mesi classe 3 in LO danno 1.540,06 come il preventivatore, e l'intera griglia LO coincide cifra per cifra con quella validata a mano da Alberto. Il coefficiente e' **costante dentro ogni fascia** (verificato su due campioni per fascia), quindi le fasce sono scaglioni netti. **L'assicurazione all risk e' gia' compresa nella rata** (0,05% mensile sull'imponibile per la LO, 0,03468% per la LF — verificato: e' esattamente la differenza tra canone con servizi e canone netto del planner): e' il vantaggio da giocarsi contro Grenke, dove l'assicurazione e' esclusa dai coefficienti e si aggiunge a parte. `BCC_ASSICURAZIONE_MENSILE` e `bccQuotaAssicurazione()`. Il **finanziamento finalizzato non dipende dalla classe di rischio** (il bene e' subito del cliente). Le **spese di istruttoria sono una tantum** e si sommano a parte (`bccSpeseIstruttoria()`), sul FF si aggiunge l'**imposta sostitutiva** 2,5‰ oltre i 18 mesi (`bccImpostaFinanziamento()`). Griglie leggibili e note operative: `~/Desktop/AREE/Nord_Est_Group/BCC/COEFFICIENTI_BCC_LOLF.md`. **Campagna tasso zero** (`BCC_TASSO_ZERO`, dal file `# BCCRL_NEWCAMPAIGN'260430.xlsx`, in vigore dal 01/04/2026): TAN azzerato per il cliente, rata = imponibile / 10. Il **fornitore** paga solo il contributo del 5,30% dell'imponibile (costante, verificato); **istruttoria 75 €, bollo 16 € e spese di incasso RID 4 €/rata sono a carico del CLIENTE** (indicazione operativa di Alberto: il planner di campagna riporta invece l'istruttoria a carico fornitore, da confermare con BCC). Implementata **solo a 10 mesi**, l'unica durata del planner di campagna: per le altre il contributo cambia e BCC non l'ha comunicato. Nel simulatore compare un riquadro **"Il conto per te, come fornitore"** con prezzo di vendita, contributo, istruttoria e **netto bonificato da BCC** (es. su 12.000 € il cliente paga 1.200/mese e al fornitore restano 11.289 €): serve al partner per decidere se proporre la campagna e quanto caricare sul prezzo. Il riquadro e' a schermo ma NON nella stampa per il cliente finale, dove compare solo "tasso zero, nessun interesse a carico del cliente". Per sospenderla: `attiva: false`.
- **Coefficienti PagaRent** (`src/data/pagarent.ts`, 03/08/2026): PagaRent non pubblica una tabella, il calcolatore rapido della dashboard e' l'unica fonte. I coefficienti sono stati **rilevati interrogando il calcolatore importo per importo** (prodotto Generale, cliente Azienda, addebito Mensile) su tutte e quattro le durate 24/36/48/60. Struttura: `canone = importo x coefficiente della fascia`, coefficiente **costante dentro la fascia** (verificato: tra 14.000 e 22.000 a 60 mesi sei campioni danno 2,34572% con intercetta zero, quindi nessuna quota fissa spalmata sul canone). **Le fasce non coincidono tra le durate**: a 48 mesi la terza fascia arriva a 10.000, a 60 mesi a 10.500, e a 24 mesi esiste una fascia stretta 21.501-22.000. Gli scaglioni nel file sono i punti misurati: dove il salto cade tra due campioni si applica il coefficiente del campione inferiore, il piu' alto, cosi' il canone mostrato non e' mai sotto quello vero. **Validazione**: su 24 combinazioni importo/durata (da 1.000 a 100.000) lo scarto massimo rispetto al calcolatore e' mezzo centesimo. Fuori dal canone restano istruttoria 100 EUR una tantum e assicurazione all risk facoltativa fino al 3,47% (come Grenke, al contrario di BCC dove e' gia' dentro la rata). **Per aggiornarli**: rifare il giro sul calcolatore quando PagaRent cambia listino. Nota: i vecchi `src/data/datron-coefficienti.json` e `duplex-coefficienti.json` restano quelli rilevati nel 2026 su importi piccoli e non vanno usati sopra i 12.000 EUR.
...
## Prospetti fotovoltaico — motore di calcolo e microservizio PDF

Alimentano il portale InnovaLux e sono pensati per essere riusati da altri fornitori fotovoltaico.

- `src/lib/prospetti-pv-coefficienti.ts` → **SERVER-ONLY**, non importarlo mai da un'isola client. **Canale PagaRent dal 07/08/2026** (prima era la griglia Rete Rent-Grenke a 4 fasce, 36/48/60/72). I coefficienti NON sono duplicati qui: arrivano da `src/data/pagarent.ts`, gia' fonte di verita' del portale UNIDIMA, quindi si aggiornano in un posto solo. Qui restano durate (24/36/48/60: PagaRent non quota i 72 mesi), riscatti, costanti fiscali (IRES 24%, IRAP 3,9%), parametri leasing (TAN 6%, riscatto 1%), agevolazioni (Sabatini Green 10,08% + 900 euro di gestione, iper 180% + 3.000 euro), modello energetico e mappa provincia -> zona. **I riscatti sono l'unico dato senza fonte PagaRent**: restano i valori Grenke (6/4/3%) su indicazione di Alberto, e il 24 mesi riusa il valore del 36. Da correggere appena PagaRent li comunica. **Conseguenza del passaggio**: i prospetti consegnati a B Service (09/07/2026) e Giaccio Cesare (03/08/2026) non sono piu' riproducibili, i canoni PagaRent sono circa il 15% piu' alti (Giaccio a 60 mesi: 1.063,20 contro 945,24), e su impianti come quello il canone non e' piu' coperto dal risparmio in bolletta. Anche `src/data/esg.ts` resta una terza tabella diversa: non allinearle senza verifica.
- `src/lib/prospetti-pv.ts` → **SERVER-ONLY**. Motore puro: canoni su tutte le durate, riscatti, bilancio energetico, fiscalita', leasing francese con valore finale attualizzato, Sabatini, iperammortamento, durata consigliata (la piu' corta in cui il canone al netto della deducibilita' e' coperto dal beneficio energetico, altrimenti 72) e generazione dei testi narrativi dai numeri reali. Gli arrotondamenti replicano passo per passo quelli del Python: il canone si arrotonda a 2 decimali PRIMA di moltiplicarlo per la durata. `euro()` usa `useGrouping: 'always'` perche' l'italiano di default non separa le migliaia sotto le cinque cifre (1450 invece di 1.450) mentre il Python le separa sempre.
- `src/lib/prospetti-pv.test.ts` → test di accettazione sui due casi validati. Girano con `npm test` (vitest, aggiunto il 07/08/2026: e' il primo test runner del progetto).
- `src/lib/preventivi-pv.ts` → orchestrazione lato server: calcola, chiama il microservizio PDF, salva PDF e record su Blob, manda la mail con i PDF in allegato. **Se il microservizio non risponde il preventivo non fallisce**: record salvato, mail inviata senza allegati, portale che mostra i numeri e avvisa.
- **Microservizio PDF**: repo separato `~/dev/mcf-prospetti-pv`, deployato su https://mcf-prospetti-pv.vercel.app/api/prospetti (progetto Vercel a se' stante, runtime Python; serve `pyproject.toml` con `[tool.vercel] entrypoint` perche' `api/` e' un package). Contiene `motore_prospetto_pv.py` (reportlab + matplotlib, resa validata contro i PDF originali: non ricostruirla in HTML). **Non e' piu' identico all'originale**: le quattro durate erano cablate in cinque punti e sono diventate la costante `DURATE`, iniettata dal portale insieme a coefficienti e riscatti; i titoli "il canone se lo paga il sole" sono diventati "quanto del canone lo paga il sole", perche' con PagaRent la copertura non e' piu' integrale piu' `assets/fonts/Manrope-*.ttf` e `assets/logo_circle_small.png`, senza i quali i PDF escono in Helvetica. Endpoint POST protetto da bearer token, riceve `{dati, tabella}` e restituisce i due PDF in base64. **Durate, coefficienti e riscatti viaggiano nel campo `tabella`**, non nel payload del PDF. I coefficienti arrivano gia' risolti per l'importo della richiesta, in un'unica fascia (gli scaglioni li legge solo il portale: cambiano tra operatori e su PagaRent perfino tra durate), quindi l'handler sostituisce anche `fascia()`. I default del motore vengono ripristinati a ogni richiesta: la funzione resta calda e una tabella iniettata sopravvivrebbe alle chiamate successive. I riscatti vanno iniettati come int quando sono interi, altrimenti il PDF stampa "3.0%" invece di "3%".

Il motore Python ricalcola i numeri per conto suo dagli stessi input: sono due implementazioni della stessa formula, tenute allineate dai test. La duplicazione e' voluta, serve al fallback.

**CSS del markup dinamico**: le regole che vestono HTML iniettato via `innerHTML` NON possono stare nel blocco `<style>` scoped di Astro, perche' lo scoping passa da un attributo `data-astro-cid-*` che gli elementi creati a runtime non hanno. Vanno in un secondo blocco `<style is:global>` con i selettori prefissati dalla classe radice della pagina. Vedi il fondo di `innovalux.astro`. Gli altri quattro portali partner hanno ancora questo difetto sulla lista "Le tue richieste".
...
- `src/pages/api/submit-full-service.ts` → endpoint del portale FULL SERVICE (`/tools/full-service`), aggiunto il 24 luglio 2026. Involucro sottile su `src/lib/pratiche-partner.ts`: passa il partner Full Service, il documento extra "Offerta / preventivo al cliente finale" e la nota di lavorazione (canale BCC Rent & Lease su beni IT e attrezzature, assicurazione già compresa nella rata). Log `form_submitted_fullservice` / `form_rejected_fullservice` / `lead_lost_fullservice`.
- `src/pages/api/submit-unidima.ts` → endpoint del portale UNIDIMA (`/tools/unidima`), aggiunto il 3 agosto 2026. Involucro sottile su `src/lib/pratiche-partner.ts`: passa il partner Unidima, il documento extra "Offerta / preventivo al cliente finale" e la nota di lavorazione (canale ReteRent/Grenke tabella Pioneer su ledwall e display, censimento fornitore in corso, Grenke paga a collaudo). Log `form_submitted_unidima` / `form_rejected_unidima` / `lead_lost_unidima`.
- `src/pages/api/preventivo-innovalux.ts` → endpoint del portale InnovaLux (`/tools/innovalux`), aggiunto il 7 agosto 2026. Involucro sottile su `src/lib/preventivi-pv.ts`. POST autenticato (bearer: password partner o chiave admin) con `{input: {...}}`: calcola i numeri, chiama il microservizio PDF, salva i due prospetti e il record su Blob sotto `preventivi/innovalux/<id>/`, manda la mail Resend con i PDF in allegato e risponde con tutti i numeri per il riepilogo a schermo. Log `preventivo_generato_innovalux`.
- `src/pages/api/preventivi-pv.ts` → storico dei preventivi di un portale fornitore. GET `?partner=<slug>` con bearer (password partner o chiave admin); risponde anche `ruolo`, usato dal gate per validare la chiave admin. Legge i record da `preventivi/<slug>/`.
...
- `PROSPETTI_PDF_URL` → URL dell'endpoint del microservizio PDF dei prospetti fotovoltaico (progetto Vercel separato, repo `~/dev/mcf-prospetti-pv`). In locale: `http://127.0.0.1:8787/api/prospetti` con `PROSPETTI_TOKEN=dev-token python scripts/serve_locale.py`.
- `PROSPETTI_TOKEN` → token condiviso col microservizio PDF. Va impostato con lo stesso valore sui due progetti Vercel. Senza, il microservizio risponde 401 e il portale degrada mostrando i soli numeri.
