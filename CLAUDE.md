...
- **Coefficienti BCC Rent&Lease** (`src/data/bcc.ts`, aggiunto 23/07/2026): BCC quota diversamente da Grenke, per **classe di rischio del bene** oltre che per durata e fascia di importo. La griglia coperta oggi e' la **locazione operativa** (5 classi × 4 durate × 5 fasce), estratta dal planner `#_BCCRLplanner_LO+LF+FF_260226.xlsx` ricalcolando il modello TIR/spread, non da tabella statica. Config della griglia: canone fisso mensile, VR 1%, anticipo primo canone, provvigione 3%, pagamento fornitore 2 giorni, ristorno spese SI. **Validata al centesimo**: 75.000 € a 60 mesi classe 3 → 1.540,05 € contro 1.540,06 € del preventivatore. Il coefficiente "con servizi" include gia' le spese di incasso SDD; le **spese di istruttoria sono una tantum** e si sommano a parte (`bccSpeseIstruttoria()`, scaglioni 75/100/150/200/300/500 €). La colonna di fascia si sceglie per vicinanza all'importo (le fasce sono importi di riferimento, non scaglioni netti: tra fasce contigue il coefficiente cambia di pochi centesimi di punto). Fonte e note operative: `~/Desktop/AREE/Nord_Est_Group/BCC/COEFFICIENTI_BCC_LOLF.md`. **Mancano ancora leasing finanziario e finanziamento finalizzato**: i coefficienti non esistono come tabella nel planner e vanno estratti da Excel cambiando tipo operazione (modulo pronto da compilare in `~/Desktop/AREE/Nord_Est_Group/BCC/DA-COMPILARE_coefficienti-LF-FF.md`). Del FF sappiamo gia' che la rata e' un ammortamento francese sul TAN (verificato sull'esempio del foglio informativo) e che esiste una campagna tasso zero con contributo fornitore 5,30%. Da rigenerare quando cambia il planner (nuovo trimestre o nuova campagna).
